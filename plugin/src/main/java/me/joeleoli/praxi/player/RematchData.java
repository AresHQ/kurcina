...
public class RematchData {
	...
	private static final HoverEvent HOVER_EVENT = new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ChatComponentBuilder(Style.YELLOW + "Click to accept this rematch invite.").create());
	...
	public void request() {
		...
		if (senderPraxiPlayer.getRematchData() == null || targetPraxiPlayer.getRematchData() == null || !senderPraxiPlayer.getRematchData().getKey().equals(targetPraxiPlayer.getRematchData().getKey())) {
			return;
		}
		...
		sender.sendMessage(Style.YELLOW + "You sent a rematch request to " + Style.PINK + target.getName() + Style.YELLOW + " on " + Style.PINK + this.arena.getName() + Style.YELLOW + ".");
		target.sendMessage( Style.PINK + sender.getName() + Style.YELLOW + " has sent you a rematch request on " + Style.PINK + this.arena.getName() + Style.YELLOW + ".");
		target.sendMessage(new ChatComponentBuilder("").parse("&6Click here or type &b/rematch &6to accept the invite.").attachToEachPart(HOVER_EVENT).attachToEachPart(CLICK_EVENT).create());

		this.sent = true;
		targetPraxiPlayer.getRematchData().receive = true;

		senderPraxiPlayer.refreshRematch();
		targetPraxiPlayer.refreshRematch();
	}
