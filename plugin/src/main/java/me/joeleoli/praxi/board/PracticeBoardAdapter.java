	...
	public List<String> getScoreboard(Player player, Board board) {
		...
		if (praxiPlayer.isInLobby()) {
			...
			if (!Praxi.getInstance().getEventManager().getEventCooldown().hasExpired()) {
				toReturn.add("Event Cooldown: " + Style.PINK + TimeUtil.millisToTimer(
			...
			if (event.isWaiting()) {
				...
				toReturn.add(" " + Style.PINK + event.getRoundPlayerA().getName() + Style.GRAY + " (" +
				             event.getRoundPlayerA().toPlayer().getPing() + " ms)");
				toReturn.add(" " + Style.PINK + event.getRoundPlayerB().getName() + Style.GRAY + " (" +
				             event.getRoundPlayerB().toPlayer().getPing() + " ms)");
			}
		}
